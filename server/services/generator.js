...
async function generateQuiz(chapter) {
    let prompt = `Create very short 5 MCQ quizzes having 4 unique options on the topic: ${chapter}. The answers will be in zero-based indexing. The format is JSON. Example: {title: "", questions: [{question:"", options: ["",""], answer: index}]}`
    return new Promise(async (resolve, reject) => {
        try {
